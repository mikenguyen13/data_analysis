...
### Johnson-Neyman 2.0: a GAM-based Two-step Workflow

Classic Johnson-Neyman probing is linear: it reports the range of moderator values $z$ over which the marginal effect of $x$ on $y$ is statistically distinguishable from zero, under the assumption that the conditional effect is linear in $z$. When the true relationship is curved, linear probing can produce qualitatively wrong conclusions, including sign reversals and spurious significance. The GAM-based variant, sometimes called Johnson-Neyman 2.0, fits a flexible interaction with a smooth-by term and translates the resulting plot into a familiar test on the relevant subset.

The workflow has two steps. Step 1 fits a GAM with a smooth-by interaction and plots the implied marginal effect of $x$ across moderator values, with confidence band. Step 2 reads a qualitative pattern from the plot (e.g. "negative for $z < z_0$") and confirms it with a conventional t-test or linear regression on exactly that subset. The two-step structure preserves interpretability, yields a single estimate and p-value per region, and avoids the strong linearity assumption.

The method is most defensible when $x$ is randomly assigned and $z$ is measured; if $x$ and $z$ are correlated, the Step-2 subset regressions are descriptive rather than causal, and the GAM plot should be emphasized over any subset point estimate. If the true interaction is linear, the GAM collapses to linear and little efficiency is lost.

```{r, eval = FALSE}
# Quick demo: binary x randomized, nonlinear true interaction in z
library(mgcv)
library(dplyr)
library(ggplot2)
library(interactions)
...
y  <- 0.2 * z + tau_z * x + rnorm(n)
dat <- tibble(y, x = factor(x), z)

# Classic linear Johnson-Neyman, for comparison
m_lin <- lm(y ~ x * z, data = dat)
jn_lin <- johnson_neyman(m_lin, pred = x, modx = z, control.fdr = FALSE)

# Step 1: fit GAM with smooth-by interaction and plot marginal effect of x across z
m_gam <- gam(y ~ s(z) + s(z, by = x) + x, data = dat, method = "REML")

grid    <- tibble(z = seq(min(z), max(z), length.out = 401))
pred_x1 <- predict(m_gam, newdata = mutate(grid, x = factor(1, levels = levels(dat$x))), se.fit = TRUE)
pred_x0 <- predict(m_gam, newdata = mutate(grid, x = factor(0, levels = levels(dat$x))), se.fit = TRUE)
eff     <- pred_x1$fit - pred_x0$fit
se      <- sqrt(pred_x1$se.fit^2 + pred_x0$se.fit^2)
crit    <- qnorm(0.975)
df_g    <- grid %>% mutate(effect = eff, lo = eff - crit * se, hi = eff + crit * se)
...
ggplot(df_g, aes(z, effect)) +
    ...
    labs(x = "Moderator z", y = "Marginal effect of x on y",
         title = "GAM Johnson-Neyman curve")

# Step 2: confirm a region with a conventional test
crosses <- with(df_g, z[which(lo <= 0 & hi >= 0)])
z0      <- if (length(crosses)) min(crosses) else min(df_g$z) - 1
sub_neg <- dat %>% filter(z < z0)
t.test(y ~ x, data = sub_neg)
```

A reusable wrapper detects sign regions automatically and applies a Benjamini-Hochberg correction across detected regions:
...
jn2_probe <- function(data, y, x, z,
    ...
    if (is.numeric(xvec)) {
        stopifnot(all(xvec %in% c(0, 1)))
        df[[rlang::as_name(x)]] <- factor(xvec, levels = c(0, 1))
    } else {
        fx <- factor(xvec)
        stopifnot(nlevels(fx) == 2)
    ...
    idx_to_ranges <- function(runs) {
        ...
        tibble(z_lo = curve_df$z[runs$start],
    ...
    confirm_tbl <- map_dfr(seq_len(nrow(regions)), function(i) {
        ...
        if (n0 < 5 || n1 < 5) return(tibble(z_lo = r$z_lo, z_hi = r$z_hi, sign = r$sign,
                                            n = nrow(sub), n_ctrl = n0, n_trt = n1,
                                            est = NA_real_, p_ttest = NA_real_,
                                            est_lm = NA_real_, p_lm = NA_real_))
        tt   <- t.test(sub[[rlang::as_name(y)]] ~ sub[[rlang::as_name(x)]])
        est  <- as.numeric(diff(tt$estimate))
        if (confirm_lm) {
            lm_fit <- lm(as.formula(paste(rlang::as_name(y), "~", rlang::as_name(x), "+", rlang::as_name(z))),
                         data = sub)
            co     <- summary(lm_fit)$coef
            est_lm <- unname(co[paste0(rlang::as_name(x), trt), "Estimate"])
            p_lm   <- unname(co[paste0(rlang::as_name(x), trt), "Pr(>|t|)"])
        } else { est_lm <- NA_real_; p_lm <- NA_real_ }
        tibble(z_lo = r$z_lo, z_hi = r$z_hi, sign = r$sign,
               n = nrow(sub), n_ctrl = n0, n_trt = n1,
    ...
    if (nrow(confirm_tbl) > 0) {
        if (!all(is.na(confirm_tbl$p_ttest))) confirm_tbl$p_ttest_adj <- p.adjust(confirm_tbl$p_ttest, method = adjust_p)
        if (!all(is.na(confirm_tbl$p_lm)))    confirm_tbl$p_lm_adj    <- p.adjust(confirm_tbl$p_lm,    method = adjust_p)
    }

    plot <- ggplot(curve_df, aes(z, effect)) +
        geom_ribbon(aes(ymin = lo, ymax = hi), alpha = 0.15) +
        geom_line(linewidth = 0.9) +
        geom_hline(yintercept = 0, linetype = 2) +
        labs(x = rlang::as_name(z),
             y = paste("Effect of", rlang::as_name(x), "on", rlang::as_name(y)),
             title = "Johnson-Neyman 2.0: GAM probing of interaction")

    list(model = m_gam, curve = curve_df, regions = regions,
         confirm = confirm_tbl, plot = plot,
         meta = list(x_levels = levs, control = ctrl, treatment = trt,
                     alpha = alpha, p_adjust = adjust_p, formula = f))
}
```

When reporting results, describe the shape from the GAM curve first ("the marginal effect is negative for low $z$, crosses zero near $z \approx 0.65$, and turns positive for high $z$") and then attach the confirmatory numbers from Step 2. For continuous $x$, replace the smooth-by term with a tensor interaction `ti(x, z)` and compute the derivative $\partial \mathbb{E}[y] / \partial x$ across $z$ in place of the difference of two predictions.
