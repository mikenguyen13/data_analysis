...
Linear in parameter and linear in endogenous variable. Table \@ref(tab:iv-2sls-linear-manual) reports the manual two-stage and control-function estimates side by side for the linear case.

```{r iv-2sls-linear-manual, fig.cap="Manual 2SLS and control-function estimates for a linear endogenous regressor, with first-stage and second-stage outputs reported side by side."}
# manual
# 2SLS
...
Nonlinear in endogenous variable. Table \@ref(tab:iv-2sls-nonlinear-endo) reports the manual two-stage and control-function estimates when the endogenous regressor enters the outcome equation nonlinearly.

```{r iv-2sls-nonlinear-endo, fig.cap="Manual 2SLS and control-function estimates when the endogenous regressor enters nonlinearly, with first-stage and second-stage outputs reported side by side."}
# 2SLS
...
Nonlinear in parameters. Table \@ref(tab:iv-2sls-nonlinear-params) reports the manual two-stage and control-function estimates when the structural equation is nonlinear in parameters.

```{r iv-2sls-nonlinear-params, fig.cap="Manual 2SLS and control-function estimates when the structural equation is nonlinear in parameters, with first-stage and second-stage outputs reported side by side."}
# 2SLS
first_stage = lm(endogenous ~ exogenous, data = my_data)
...
Figure \@ref(fig:iv-shiny-late-simulation) describes a Shiny app that simulates the IV model and displays first-stage, reduced-form, calculated-LATE, and IV-LATE coefficients with confidence intervals as the user varies the true effect, the first-stage strength, and the direct effect of the instrument on the outcome.

```{r iv-shiny-late-simulation, fig.cap="Interactive Shiny simulation of the IV model: first-stage, reduced-form, calculated-LATE, and IV-LATE coefficients with confidence intervals as the user varies the true effect, the first-stage strength, and the instrument's direct effect on the outcome.", eval = FALSE}
