...
While IVs can provide a solution to endogeneity, the practical difficulties roughly cluster into three families: assumption failures (an exclusion restriction that does not actually hold, or an instrument that is too weak to identify anything reliably), interpretation traps (LATE applies only to compliers, not to the population the reader probably has in mind), and design hygiene (the same instrument used many times across literatures eventually becomes implausible). The list below details each in turn; in any given application, the exclusion restriction and weak-instrument concerns are usually the ones that decide whether the analysis is publishable.
...
IV (this chapter) and [matching](#sec-matching-methods) (Chapter 35) are the two remaining strategies when neither a cutoff nor panel structure is available. They rest on very different identifying assumptions, and they answer slightly different questions; Table \@ref(tab:iv-vs-matching) lays them side by side.
