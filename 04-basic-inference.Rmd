...
For continuous distributions, the Bhattacharyya distance can be approximated using numerical integration or discretization, as illustrated in Figure \@ref(fig:bi-bhattacharyya-continuous).

```{r bi-bhattacharyya-continuous, fig.cap="Histogram-based approximation of the Bhattacharyya coefficient and distance between two simulated continuous distributions, illustrating how the overlap of binned densities translates into a similarity score."}
...
For continuous distributions, the TV distance can be approximated using discretization or numerical integration. Figure \@ref(fig:bi-tv-distance-continuous) demonstrates this using random samples.

```{r bi-tv-distance-continuous, fig.cap="Histogram-based approximation of the total variation distance between two simulated continuous distributions, showing how discretized densities yield an interpretable measure of probability-mass divergence."}
# Generate two continuous distributions
set.seed(1)
P_continuous <-
    rnorm(1000, mean = 0, sd = 1)  # Standard normal distribution
