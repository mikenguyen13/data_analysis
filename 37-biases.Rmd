...
In econometrics, the main objective is often to uncover causal relationships. The biases that frustrate that objective come from at least three structurally distinct sources: how the data are generated (endogeneity, selection, measurement error), how they are collected (aggregation, survivorship, sample selection), and how the literature is curated (publication bias). Mixing these up makes a fix that helps in one setting useless in another, so it is worth keeping the categories straight even when the names overlap. The list below organises the biases this book has discussed alongside those treated here.
...
: (#tab:p-hacking-discourse) Discourse on $p$-hacking and publication bias in the literature.
