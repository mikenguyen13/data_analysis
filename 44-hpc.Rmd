...
::: {.chapter-abstract}
**Abstract.** Analyses that once fit comfortably on a laptop now routinely exceed it, whether through data volume, simulation scale, or the resampling that modern inference demands. This chapter covers high-performance computing for applied data analysis in R, treating it as an engineering discipline rather than a matter of requesting more cores. It reviews the parallel computing ecosystem, including the parallel package, foreach with doParallel, the future framework and its interchangeable backends, BiocParallel, snow, and sparklyr for distributed processing through Apache Spark. The emphasis falls on the practices that separate a real speedup from a wasted cluster allocation: profiling before parallelizing so that effort goes to the actual bottleneck, choosing a task granularity coarse enough that communication overhead does not dominate, estimating memory and runtime before submitting a job, testing on a small subset first, and monitoring resource use while the job runs. A worked R workflow and a demonstration make these recommendations concrete.

**Keywords:** high-performance computing; parallel computing; future; foreach; Apache Spark; distributed computing; profiling; scalability; resource estimation
:::
