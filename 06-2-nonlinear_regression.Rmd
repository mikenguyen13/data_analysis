...
summary(penicillin)
library(ggplot2)
ggplot(penicillin, aes(y=yield, x=treat, shape=blend, color=blend)) + # treatment = fixed effect, blend = random effects
    geom_point(size=3) + 
    xlab("Treatment") 
```
...
$$
E(y_i) = E(\exp(\mathbf{x_i' \beta + z_i' \alpha})) = \exp(\mathbf{x'_i \beta})
$$
