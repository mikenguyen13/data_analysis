...
-   For [Zero-valued Outcomes], we can't directly interpret the treatment coefficient of log-transformed outcome regression as percentage change [@chen2023logs]. To have percentage change interpretation, we can either do:
...
-   To address this:
    ...
    -   Distortions persist even in single instrument cases, seen in:
...
[@lee2022valid] proposes a new method that is aligned better with traditional econometric training than AR, where it is called the tF procedure. It incorporates both the 1st-stage F-stat and the 2SLS t-ratio.
...
3.  **Robustness**:
    ...
    -   Robust against common error structures (e.g., heteroskedasticity or clustering).
...
Traditional T-ratio Inference:

-   In large samples, $\hat{t}^2 \to^d t^2$
-   Standard normal critical values are ±1.96 for 5% significance level testing.
...
# To get the correct SE based on 1st-stage F-stat
