...
Several practical features make DID popular in applied work. The visuals are intuitive: a two-line plot can immediately signal whether pre-treatment trends look parallel. The data demands are light, since only two time points and a clear treatment onset are needed. The basic estimator extends naturally, with regression frameworks accommodating staggered treatments, multiple periods, or continuous exposures. And the design is transparent: assumptions and identifying variation are easy to communicate to executives, regulators, or reviewers.
...
: (#tab:did-estimator-illustration-state-level) DiD estimator illustration using a state-level example.
