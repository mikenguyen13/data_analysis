...
To check pre- and post-treatment dynamics, we ask for an event study via `type = "event"`. This shows how the outcome evolves *around* the adoption time. Negative "event" values correspond to pre-treatment, while nonnegative "event" values are post-treatment. Table \@ref(tab:nldid-event-study) reports the dynamic ATT estimates by relative period.

```{r nldid-event-study, fig.cap="ETWFE event-study estimates by relative event time, with cluster-robust standard errors at the unit level."}
...
If you like to see a clean numeric summary of the *dynamic* estimates by period, you can pipe your event-study object into `modelsummary`. Table \@ref(tab:nldid-event-modelsummary) renders the resulting wide-format table.

```{r nldid-event-modelsummary, fig.cap="ETWFE event-study by relative adoption period rendered as a wide-format modelsummary table with cluster-robust standard errors at the store level.", eval = FALSE}
