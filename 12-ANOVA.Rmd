...
$$
Y = \mu_{..} + \sum_{i=1}^{a-1} \alpha_i u_i + \sum_{j=1}^{b-1} \beta_j v_j + \sum_{i=10}^
$$
...
$$
So $Y_{ijk} \sim N(\mu_{..},\sigma^2_\alpha + \sigma^2_\beta +  \sigma^2_{\alpha \beta} + \sigma^2)$  
...
Mean Square | df | Fixed ANOVA \ (A, B fixed) | Random ANOVA \ (A, B random) | Mixed ANOVA \ (A fixed, B random) 
---|---|---|---|---
MSA | a -1 | $\sigma^2 + nb \frac{\sum \alpha_i^2}{a-1}$ | $\sigma^2 + nb \sigma^2_\alpha + n \sigma^2_{\alpha \beta}$ | $\sigma^2 + nb \frac{\sum \sigma^2}{a-1} + n\sigma^2_{\alpha \beta}$ 
MSB | b -1 | $\sigma^2 + na \frac{\sum\beta^2_j}{b-1}$ | $\sigma^2 + na \sigma^2_\beta+ n \sigma^2_{\alpha \beta}$ | $\sigma^2 + na \sigma^2_{\beta}$ 
MSAB | (a-1)(b-1) | $\sigma^2 + n \frac{\sum \sum (\alpha \beta)^2_{ij}}{(a-1)(b-1)}$ | $\sigma^2 + n \sigma^2_{\alpha \beta}$ | $\sigma^2 + n \sigma^2_{\alpha \beta}$ 
MSE | (n-1)ab | $\sigma^2$ | $\sigma^2$ | $\sigma^2$ 
...
We could use the power tablse directly when effects are fixed and design is balanced by using **minimum range** of factor level means for your desired differences  
...
Treatments | $n\sum_j(\bar{Y}_{.j}-\bar{Y}_{..})^2$ | r - 1 | $\sigma^2 + n \frac{\sum \tau^2_j}{r-1}$ | $\sigma^2 + n \sigma^2_\tau$ 
...
Factor B | SSB(A) | a(b-1) | MSB(A) | $\sigma^2 + n \frac{\sum \sum \beta_{j(i)}^2}{a(b-1)}$
Error | SSE | ab(n-1) | MSE | $\sigma^2$
Total | SSTO | abn -1 | | 

Degree of freedom  

 * Factor A: For any level of factor A, there are b(n-1) df associated with the error sum of squares Aggregating over all levels of factor A, there are ab(n-1) df associated with SSE  
 * Factor: For any level of factor A, there are b-1 df associated with the factor B sum of squares.Aggregating over all levels of factor A, there are  a(b-1) df associated with SSB(A)  
...
---|---|---
MSA | $\sigma^2 + bn \frac{\sum \alpha_i^2}{a-1} + n \sigma^2_\beta$ | $\sigma^2 + bn \sigma^2_\alpha + n \sigma^2_\beta$
MSB(A) | $\sigma^2 + n \sigma^2_\beta$ | $\sigma^2 + n \sigma^2_\beta$
MSE | $\sigma^2$ | $\sigma^2$
**Test Statistics** | | 
Factor A | $\frac{MSA}{MSB(B)}$ | $\frac{MSA}{MSB(A)}$
