...
```{r fig-sim-function, fig.cap="Simulated Data with True Regression Function", fig.alt="Scatter plot with blue data points and red dashed line showing wavy true regression function", out.width="100%", fig.align="center"}
...
```{r Nadaraya–Watson Kernel Estimator}
...
```{r Priestley–Chao Kernel Estimator}
...
```{r fig-pc-kernel-est, fig.cap='Priestley–Chao Kernel Estimator', fig.alt='Scatter plot illustrating the Priestley–Chao Kernel Estimator with a bandwidth of 0.5. The x-axis represents values from 0 to 10, and the y-axis shows estimated m(x) values. Blue dots indicate data points, while an orange line represents the kernel estimate. A red dashed line shows a comparison or confidence interval. '}
...
```{r Gasser–Müller Kernel Estimator}
...
```{r fig-comp-kernel, fig.cap="Comparison of Kernel-Based Regression Estimators",   fig.alt="Chart titled Comparison of Kernel-Based Regression Estimators showing estimated m of x against x. The x-axis ranges from 0 to 10 and the y-axis from -3 to 2. Four lines represent different estimators: Gasser-Mueller in purple, Nadaraya-Watson in green, Priestley-Chao in orange, and the true function in red dashed. Data points are shown as gray dots. The chart highlights variations in estimation methods", out.width="100%", fig.align="center"}
...
```{r Cross-Validation for Bandwidth Selection (Optional)}
...
```{r fig-cv-bandwidth-selection, fig.cap="Cross-Validation for Bandwidth Selection", fig.alt="Chart titled Cross-Validation for Bandwidth Selection showing a plot of cross-validation error against bandwidth h. The curve is U-shaped, starting high, dipping to a minimum, and then rising again. A red dot marks the minimum point on the curve, indicating the optimal bandwidth selection",   out.width="100%", fig.align="center"}
# Plot CV errors
...
```{r fig-sim-func, fig.cap = "Simulated Data with True Regression Function", fig.alt = "Scatter plot titled Simulated Data with True Regression Function. X-axis labeled x ranges from 0 to 10. Y-axis labeled y ranges from -1 to 1. Blue dots show data points. A red dashed line shows a wavy regression function.", out.width = "100%", fig.align = "center"}
# Visualization of the data
ggplot(data.frame(x, y), aes(x, y)) +
    geom_point(color = "darkblue") +
    geom_line(aes(y = true_function(x)),
...
```{r Local Polynomial Regression Implementation}
# Gaussian Kernel Function
gaussian_kernel <- function(u) {
    (1 / sqrt(2 * pi)) * exp(-0.5 * u ^ 2)
}
...
```{r Visualization of Local Polynomial Fits}
...
```{r fig-comp-local-reg, fig.cap="Comparison of Local Regression", fig.alt="Two panel XY chart comparing local linear and quadratic regression models. The left panel shows local linear regression with p equal to 1 and includes a green line for the estimated function and red dashed lines for confidence intervals. The right panel shows local quadratic regression with p equal to 2, using an orange line for the estimate and red dashed lines for confidence intervals. Both panels use a bandwidth of 0.8 and plot estimated m of x against x, with scattered blue data points", out.width="100%", fig.align="center"}
...
```{r Bandwidth Selection via Cross-Validation}
...
```{r Plotting Cross-Validation Results}
...
```{r fig-cv-bandwith-selection, fig.cap="Cross-Validation for Bandwidth Selection", fig.alt="Line chart titled Cross-Validation for Bandwidth Selection showing cross-validation error against bandwidth h. Two curves represent linear regression with p equal to 1 in green and quadratic regression with p equal to 2 in orange. Red points on the curves indicate optimal bandwidths. A legend identifies the regression degree types", out.width="100%", fig.align="center"}
...
```{r Final Fit with Optimal Bandwidths}
...
```{r fig-line-comp-linear-quad, fig.cap="Line Comparison between Linear and Quadratic Estimates", fig.alt="Line chart displaying estimated functions compared to a true function. The x-axis is labeled x and ranges from 0 to 10. The y-axis is labeled estimated m of x and ranges from -1 to 1. The chart includes three lines: a green line for the linear estimate, an orange line for the quadratic estimate, and a red dashed line for the true function. Gray data points are scattered throughout. A legend on the right identifies the lines", out.width="100%", fig.align="center"}
# Plot final fits
ggplot() +
    geom_point(aes(x, y), color = "gray60", alpha = 0.5) +
    geom_line(
...
```{r fig-sim-function-alt, fig.cap="Simulated Data with True Regression Function", fig.alt="Scatter plot titled Simulated Data with True Regression Function showing blue data points distributed along the x-axis from 0 to 10 and the y-axis from -1 to 1. A red dashed line represents the true regression function, illustrating a non-linear trend with peaks and troughs. The x-axis is labeled x and the y-axis is labeled y", out.width="100%", fig.align="center"}
# Visualization of the data
ggplot(data.frame(x, y), aes(x, y)) +
...
```{r Generalized Cross-Validation (GCV) for Optimal lambda}
...
```{r fig-gcv-smoothing-selection, fig.cap="GCV for Smoothing Parameter Selection", fig.alt="Graph titled GCV for Smoothing Parameter Selection showing the relationship between the smoothing parameter spar on the x-axis and the GCV score on the y-axis. The curve is blue with a red dot marking the minimum point at spar equal to 0.7, indicating the optimal smoothing parameter. The GCV score ranges from 0.2 to 0.6", out.width="100%", fig.align="center"}
# GCV Plot
...
```{r fig-spline-optimal-lambda, fig.cap="Smoothing Spline with Optimal Smoothing Parameter",  fig.alt="Scatter plot showing a smoothing spline with an optimal smoothing parameter of 0.7. The x-axis is labeled x and the y-axis is labeled estimated m of x. Gray data points are scattered throughout the plot. A green line represents the smoothing spline and a red dashed line indicates the true function", out.width="100%", fig.align="center"}
...
```{r fig-scatter-effect-x, fig.cap="Scatter Plot Effect of X", fig.alt="Three-panel scatter plots showing effects of x1, x2, and x3 on y with sine, log, and quadratic patterns respectively", out.width="100%", fig.align="center"}
# Display plots side by side
grid.arrange(p1, p2, p3, ncol = 3)
```

```{r Fitting a Generalized Additive Model}
...
```{r fig-partial-effect-x1, fig.cap="Partial Effect of x1", fig.alt="Line chart titled Partial Effect of x1 showing the effect on y as a function of x1. The curve is sinusoidal, starting at a moderate level, peaking around x1 equal to 2.5, dipping near x1 equal to 5, and rising again around x1 equal to 7.5 before slightly declining. The y-axis is labeled Effect on y and ranges from 3.5 to 5.5. The x-axis is labeled x1 and ranges from 0 to 10. The line is blue and smooth", out.width="100%", fig.align="center"}
...
```{r Model Selection: Generalized Cross-Validation (GCV) and AIC}
...
```{r fig-linear-predictor, fig.cap="Linear Predictor", fig.alt="Contour map titled Linear Predictor displaying color gradients from green to red representing varying values. Contour lines are labeled with values ranging from 2.5 to 5.5. The x-axis is labeled x1 and ranges from 0 to 10. The y-axis is labeled x2 and ranges from 0 to 5", out.width="100%", fig.align="center"}
...
```{r GAM for Logistic Regression (Binary Outcome)}
...
```{r Model Diagnostics, fig.cap="Model Diagnostics", fig.alt='Four-panel figure showing residual analysis for a statistical model. Top left: Q-Q plot of deviance residuals against theoretical quantiles, with points following a diagonal line. Top right: Scatter plot of residuals versus linear predictor, showing a random distribution. Bottom left: Histogram of residuals, centered around zero, indicating normal distribution. Bottom right: Scatter plot of response versus fitted values, showing a linear relationship.'}
# Diagnostic plots
par(mfrow = c(2, 2))
gam.check(gam_model)
par(mfrow = c(1, 1))
...
```{r fig-scatter-effects, fig.cap = "Scatter Plots of Variable Effects", fig.alt = "Three-panel scatter plot showing effects of x1 x2 and x3 on y. Each panel shows data points with different patterns. The third panel has points mostly at the top with some lower values. Axes are labeled x and y", out.width = "100%", fig.align = "center"}
grid.arrange(p1, p2, p3, ncol = 3)
```

```{r Fitting a Regression Tree}
...
```{r, fig.cap='Regression Tree', fig.alt='A regression tree diagram illustrating decision nodes and outcomes. The root node starts with a value of 2.6, splitting based on the condition of whether x3 is greater than or equal to 7.7. If yes, the left branch leads to a terminal node with a value of -4.6, while the right branch continues to further split if the condition is not met. Key decision points include conditions like whether x3 is smaller than 3, whether x3 is greater than or equal to 6.3, and whether x2 is smaller than  1.1. Terminal nodes display values such as 0.4, 2.1, 3.3, 3.6, 5, and 5.3, with corresponding sample sizes and percentages. The diagram visually represents the decision-making process in a regression analysis.'}
...
```{r, fig.cap='Pruned Regression Tree Diagram', fig.alt='Pruned regression tree diagram illustrating decision nodes and leaf nodes. The root node splits based on the condition of whether x3 is greater than or equal to 7.7, with subsequent branches dividing further based on conditions like whether x3 is smaller than 3, whether x3 is greater than or equal to 6.3, and whether x2 is smaller than 1.1. Each node displays a value, sample size n, and percentage. The tree ends with leaf nodes showing final values and sample distributions.'}
...
```{r fig-oob-error-plot, fig.cap="Out-of-Bag Error for Random Forest", fig.alt="Line chart titled Out-of-Bag Error for Random Forest showing the error rate on the y-axis and the number of trees on the x-axis. The error rate decreases sharply as the number of trees increases and stabilizes around 500 trees", out.width="100%", fig.align="center"}
...
```{r Model Comparison: Regression Tree vs. Random Forest}
...
```{r fig-tree-vs-rf, fig.cap="Regression Tree vs. Random Forest", fig.alt="Chart comparing regression tree and random forest predictions. The x-axis is labeled x1 and the y-axis is labeled predicted y. A blue dashed line represents the pruned tree and a green line represents the random forest. Gray dots indicate data points. The chart title is Regression Tree vs. Random Forest, and the legend indicates blue for the pruned tree and green for the random forest", out.width="100%", fig.align="center"}
# Visualization
ggplot() +
    geom_point(aes(x1, y),
               data = data_tree,
...
```{r Out-of-Bag (OOB) Error vs. Cross-Validation}
...
```{r fig-noisy-true-signal, fig.cap="Noisy Data with Underlying True Signal", fig.alt="Chart titled Noisy Data with Underlying True Signal showing an XY plot. The x-axis is labeled x and ranges from 0 to 1. The y-axis is labeled Signal and ranges from -1 to 2. A grey line represents noisy data with fluctuations, while a blue dashed line indicates the smoother underlying true signal", out.width="100%", fig.align="center"}
...
```{r Apply Discrete Wavelet Transform (DWT)}
...
```{r Inverse DWT for Signal Reconstruction}
...
```{r Visualization of Wavelet Denoising}
...
```{r Evaluation: Mean Squared Error (MSE)}
...
```{r fig-scatter-panel-effects, fig.cap="Scatter Plot", fig.alt="Three panel XY chart showing the effects of variables x1, x2, and x3 on y. Each panel displays a scatter plot with the x-axis labeled as x1, x2, and x3 respectively, and the y-axis labeled as y. The data points are scattered, showing varying relationships between each x variable and y. The charts are titled Effect of x1, Effect of x2, and Effect of x3", out.width="100%", fig.align="center"}
gridExtra::grid.arrange(p1, p2, p3, ncol = 3)
```

```{r Multivariate Kernel Regression (Nadaraya-Watson)}
...
```{r, fig.cap='Multivariate Kernel Regression (x3 fixed)', fig.alt='Heatmap illustrating multivariate kernel regression with x3 fixed. The x-axis represents x1, and the y-axis represents x2, both ranging from 0 to 5. The color gradient transitions from purple to yellow, indicating prediction values from 2 to 6, as shown in the legend on the right.'}
...
```{r Thin-Plate Splines (with mgcv)}
...
```{r Tensor Product Splines (for Structured Interactions)}
...
```{r Additive Models (GAMs) to Mitigate Curse of Dimensionality}
# Additive Model (GAM)
...
```{r Radial Basis Functions (RBFs) for Multivariate Regression}
...
```{r, fig.cap='Radial Basis Function Regression (x3 fixed)', fig.alt='Heatmap illustrating Radial Basis Function Regression with x3 fixed. The x-axis represents x1, and the y-axis represents x2. The color gradient ranges from purple to yellow, indicating varying levels of the predicted RBF values, labeled as pred_rbf, with a scale from 2 to 6.'}
# Visualization
...
```{r Model Comparison: Mean Squared Error (MSE)}
