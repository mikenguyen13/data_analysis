...
3.  **Local Weighting Principle**
    -   At a target location $x$, a kernel function $K\left(\frac{x - x_i}{h}\right)$ down-weights data points $(x_i)$ that are farther from $x$. Nearer points have larger kernel values, hence exert greater influence on the local estimate.
