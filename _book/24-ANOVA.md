...
## Linear Model Interpretation of ANOVA
...
The understandings of the [Single Factor Fixed Effects Model](#single-factor-fixed-effects-model-1) and the [Single Factor Random Effects Model] are different, the ANOVA is same for the one factor model. The difference is in the expected mean squares
...
And we can also compare factor means like the [Single Factor Fixed Effects Model](#single-factor-fixed-effects-model-1) using [Tukey], [Scheffe], [Bonferroni].
