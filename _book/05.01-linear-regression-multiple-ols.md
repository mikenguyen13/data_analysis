...
$$
\begin{aligned}\|\mathbf{Y}\|^2 &= \mathbf{Y}^\top \mathbf{Y} && \text{(definition of norm squared)} \\[6pt]&= (\mathbf{Y} - \mathbf{X}\hat{\beta} + \mathbf{X}\hat{\beta})^\top(\mathbf{Y} - \mathbf{X}\hat{\beta} + \mathbf{X}\hat{\beta}) && \text{(add and subtract } \mathbf{X}\hat{\beta} \text{)} \\[6pt]&= (\mathbf{Y} - \mathbf{X}\hat{\beta})^\top(\mathbf{Y} - \mathbf{X}\hat{\beta}) + 2(\mathbf{X}\hat{\beta})^\top(\mathbf{Y} - \mathbf{X}\hat{\beta}) + (\mathbf{X}\hat{\beta})^\top(\mathbf{X}\hat{\beta}) && \text{(expand } (a+b)^\top(a+b) \text{)} \\[6pt]&= \|\mathbf{Y} - \mathbf{X}\hat{\beta}\|^2 + 2(\mathbf{X}\hat{\beta})^\top(\mathbf{Y} - \mathbf{X}\hat{\beta}) + \|\mathbf{X}\hat{\beta}\|^2 && \text{(rewrite each quadratic form as a norm)} \\[6pt]&= \|\mathbf{Y} - \mathbf{X}\hat{\beta}\|^2 + \|\mathbf{X}\hat{\beta}\|^2 && \text{(orthogonality: } (\mathbf{X}\hat{\beta})^\top(\mathbf{Y}-\mathbf{X}\hat{\beta}) = 0\text{)} \\[6pt]&= \|\mathbf{X}\hat{\beta}\|^2 + \|\mathbf{Y} - \mathbf{X}\hat{\beta}\|^2.\end{aligned}
$$
...
<img src="05.01-linear-regression-multiple-ols_files/figure-html/fig-qqplot-example-1.png" alt="Q-Q plot comparing sample quantiles to theoretical quantiles. The plot shows data points forming an approximately linear pattern, indicating that the sample distribution is similar to the theoretical distribution. The x-axis represents theoretical quantiles, and the y-axis represents sample quantiles, with values ranging from -2 to 2 on the x-axis and 0 to 100 on the y-axis." width="100%" />
<p class="caption">(\#fig:fig-qqplot-example)Q-Q Plot for Assessing Normality of Sample Data</p>
</div>
...
<div class="figure" style="text-align: center">
<img src="05.01-linear-regression-multiple-ols_files/figure-html/fig-residual-plot-1.png" alt="Scatter plot titled 'Residual Plot' with residuals on the y-axis and fitted values on the x-axis. The data points are scattered around a horizontal red line at zero, illustrating the distribution of residuals. The x-axis ranges approximately from 10 to 30, and the y-axis from -4 to 6. This plot is used to assess the adequacy of model fit and check for patterns such as heteroscedasticity or non-linearity." width="100%" />
<p class="caption">(\#fig:fig-residual-plot)Residual Plot for Linear Model Diagnostics</p>
</div>
...
<div class="figure" style="text-align: center">
<img src="05.01-linear-regression-multiple-ols_files/figure-html/fig-pacf-residuals-1.png" alt="Partial autocorrelation function (PACF) plot of residuals showing lags on the x-axis and partial autocorrelation coefficients on the y-axis. Most bars fall within the horizontal blue dashed confidence bounds, except for a notable spike at lag 1 and minor spikes at lags 4 and 8. The plot is used to detect the presence of partial autocorrelation in residuals after fitting a model." width="100%" />
<p class="caption">(\#fig:fig-pacf-residuals)PACF of Residuals</p>
</div>
...
<div class="figure" style="text-align: center">
<img src="05.01-linear-regression-multiple-ols_files/figure-html/fig-moran-scatterplot-1.png" alt="Scatterplot titled 'Moran's Scatterplot' showing the relationship between raw model residuals on the x-axis and spatially lagged residuals on the y-axis. A fitted diagonal line indicates the degree of spatial autocorrelation. Most points are clustered around the origin, with a few labeled outliers such as points 10, 19, 23, and 26. Dashed vertical and horizontal lines represent zero on each axis. The plot is used to assess spatial dependence in model residuals." width="100%" />
<p class="caption">(\#fig:fig-moran-scatterplot)Moran's Scatterplot</p>
</div>
