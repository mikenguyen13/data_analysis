...
Table \@ref(tab:report-stargazer-attitude) reports stargazer-formatted regression and correlation output for the `attitude` dataset.

```{r report-stargazer-attitude, fig.cap="Stargazer output for the attitude dataset: descriptives, two OLS models, a probit model, and a correlation matrix.", message=FALSE}
...
Table \@ref(tab:report-stargazer-latex) shows the LaTeX-formatted stargazer template that can be enabled when producing a typeset manuscript.

```{r report-stargazer-latex, eval=FALSE, fig.cap="Template LaTeX stargazer output combining two OLS regressions and a probit model, with custom dependent-variable and covariate labels."}
...
Table \@ref(tab:report-apa-flextable) demonstrates an APA-styled flextable rendering of a five-row, five-column slice of `mtcars`.

```{r report-apa-flextable, eval = FALSE, fig.cap="APA-themed flextable output for the first five rows and five columns of mtcars."}
data("mtcars")
library(flextable)
theme_apa(flextable(mtcars[1:5,1:5]))
```

You can export data frames to LaTeX via `xtable` and ready-made styles via `stargazer`. (Ensure output directory exists.) Table \@ref(tab:report-export-latex) shows the xtable-and-stargazer export idiom.

```{r report-export-latex, eval=FALSE, fig.cap="Exporting mtcars to LaTeX via xtable and to AER-style stargazer output."}
...
Table \@ref(tab:report-stargazer-latex-demo) gives the LaTeX-ready stargazer template for the same three models, with custom dependent-variable and covariate labels.

```{r report-stargazer-latex-demo, eval = FALSE, fig.cap="LaTeX-ready stargazer template for the attitude OLS and probit regressions, with relabeled dependent variables and covariates."}
...
Table \@ref(tab:report-stargazer-ascii) renders the same two OLS regressions as an ASCII (`type = "text"`) stargazer table with 90% confidence intervals.

```{r report-stargazer-ascii, fig.cap="ASCII stargazer output for the attitude OLS regressions, reporting 90% confidence intervals on a single row.", message=FALSE}
...
Table \@ref(tab:report-stargazer-corr) shows the matching stargazer-formatted correlation matrix for the three core `attitude` variables.

```{r report-stargazer-corr, eval = FALSE, fig.cap="Stargazer-formatted correlation matrix for the rating, complaints, and privileges variables in the attitude dataset."}
# Correlation table
correlation.matrix <-
...
Pro Tip: Save figures reproducibly for journals. Figure \@ref(fig:report-ggsave-template) shows the canonical `ggsave` call to write the most recent ggplot to disk at journal-appropriate dimensions and resolution.

```{r report-ggsave-template, eval = FALSE, fig.cap="Canonical ggsave invocation to export the most recent ggplot at 6.5 by 4.5 inches and 300 dpi for journal submission."}
