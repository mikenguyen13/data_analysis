...
# Since single imputation underestimates standard errors,
...
The `md.pairs()` summary in Figure \@ref(fig:imp-md-pairs-anscombe) tabulates the four observed/missing combinations for every pair of variables.

```{r imp-md-pairs-anscombe, fig.cap="Pairwise missing-data patterns produced by mice::md.pairs() on the modified Anscombe dataset, summarising rr/rm/mr/mm counts for every variable pair to support imputation planning."}
...
##### Evidence for heteroscedastic data
...
```{r fig-missing-data-heatmap3, fig.cap='Heatmap', fig.alt='Heatmap showing missing data across different variables. The x-axis lists variables. The y-axis represents observations. Dark bars indicate missing data, with a legend showing 4.1% missing and 95.9% present data.'}
# Visualizing missing data
vis_miss(airquality)
```
...
```{r fig-missingness-pattern-barchart, fig.cap="Missingness Patterns", fig.alt="Bar chart showing intersection sizes for missing data in Solar and Ozone. The y-axis shows intersection sizes up to 40, with bars labeled 35, 5, and 2. The x-axis shows set sizes, with Ozone larger than Solar", out.width="100%", fig.align="center"}
# Missingness patterns using an upset plot
gg_miss_upset(airquality)
```

Figure \@ref(fig:imp-naniar-extra-plots) sketches three additional `naniar` displays (faceted scatter, missingness-by-variable, missingness-by-factor) that practitioners typically add when grouping structure is present.

```{r imp-naniar-extra-plots, eval = FALSE, fig.cap="Illustrative naniar visualisations not run in the build, showing how to extend missing-data diagnostics with faceted scatterplots, per-variable counts, and factor-grouped missingness summaries."}
...
```{r fig-barplot-and-matrix-missingness, fig.cap='Barplot and Matrix plot', fig.alt='Bar chart showing missing data percentages for four variables: Petal Width, Sepal Length, Petal Length, and Sepal Width. Petal Width has the highest missing data at 0.12, followed by Sepal Length and Petal Length at around 0.10, and Sepal Width at 0.08. Adjacent to the bar chart is a pattern matrix with yellow and blue squares, indicating data presence and absence patterns, with corresponding percentages ranging from 0.0067 to 0.6667.'}
# Plot missing values
aggr(
  iris.mis,
...
```{r fig-density-plot-imputation, fig.cap='Density Plot', fig.alt='Density plots showing distributions of sepal and petal dimensions from a dataset. The top left plot displays Sepal Length, the top right shows Sepal Width, the bottom left illustrates Petal Length, and the bottom right depicts Petal Width. Each plot features multiple overlapping red and blue curves. Each red curve represents one imputed dataset. The blue curve represents the observed dataset. The x-axis varies by plot, while the y-axis represents density.'}
