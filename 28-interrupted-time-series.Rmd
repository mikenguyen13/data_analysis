...
Table: (\#tab:its-differences-rdit-its) Differences between RDiT and ITS.
...
Table: (\#tab:its-instead-of-rd) When RDiT is used instead of traditional RD.
...
Table: (\#tab:its-comparison-other-methods) Comparison with other methods.
...
```{r its-full-time-series, fig.cap='Full time series with structural break at $T^*$.', fig.alt='Scatter plot of outcome Y against time t, with data points forming an upward trajectory. A vertical black line at t = 100 marks the location of a true jump in the outcome variable. Before and after the break, the trend remains positive, but there is a clear increase in level at the cutoff.'}
# -------------------------------------------------------------------
...
```{r its-local-linear-reg, fig.cap='Local linear regression around the cutoff $T^*$.', fig.alt='Scatter plot of outcome Y versus time t, focused around t = 100. Data points are fitted with two separate linear trends, one before and one after the vertical line at the cutoff T*. The outcome increases over time on both sides, with a visible jump at the cutoff.'}
# -------------------------------------------------------------------
...
Table: (\#tab:its-scenarios) Scenarios for ITS.
...
Figure \@ref(fig:its-full-time-series-its) shows the dataset and highlights the intervention.
