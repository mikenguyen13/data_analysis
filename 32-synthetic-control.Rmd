...
The motivating problem is awkwardly common. A single state passes a tax reform, a single firm rolls out a new pricing policy, a single country adopts a peg — and you want to know what would have happened in the absence of the intervention. With one treated unit, the comparison-group machinery of [difference-in-differences](#sec-difference-in-differences) is brittle: there is no obvious "control state" or "control firm" that is a clean counterfactual, and any single donor you pick is as much a research-design choice as the analysis itself. Synthetic control responds by replacing the *judgment* of a control with a *construction*: rather than picking one comparable unit, build a weighted average of many, where the weights are chosen so that the synthetic unit tracks the treated unit's outcome and characteristics during the pre-treatment period as closely as possible. The treatment effect is then read off as the gap between the treated unit and its synthetic shadow after the intervention.

The setting where this earns its keep is one where (i) you have a clear and discrete policy event to evaluate — a minimum wage law, a tax reform, a major advertising campaign — and (ii) there is essentially one unit on the treated side and several plausible controls on the other. Marketing and business applications have multiplied in exactly these settings. @tirunillai2017 use SCM to isolate the effect of offline TV advertising on online chatter; @wang2019mobile estimate how mobile hailing technology shifted drivers' hourly earnings; @guo2020let evaluate the Massachusetts Open Payment Law's effect on physician prescription behavior; and @adalja2023gmo exploit Vermont's GMO-labeling law to ask whether mandatory disclosure changed consumer demand. Each of these has the SCM signature: one treated case, several donor candidates, and a sharp moment of intervention.

## What the method buys you

The interesting thing about SCM is not just that it provides an answer in this awkward setting, but *what kind* of answer it provides. Three properties are worth highlighting because they help explain the method's appeal.

First, the construction is data-driven rather than judgment-driven. The donor pool's weights are chosen by an optimization that minimizes pre-treatment fit; the analyst does not get to nudge the comparison unit toward a preferred outcome. As @arkhangelsky2023large show, when the pre-treatment period is long enough, the resulting estimator delivers asymptotically normal parameters in linear panel models, putting it on the same theoretical footing as DiD.

Second, SCM matches on something stronger than covariates: it matches on pre-treatment *outcomes*. This is what distinguishes it from ordinary [matching methods], which align units on $X$ and hope the unobservables cooperate. By forcing the synthetic unit to track the treated unit's outcome trajectory before treatment, SCM absorbs whatever combination of observed and unobserved factors generated that trajectory — at least to the extent they were operating in the donor pool. A Bayesian variant ([Bayesian Synthetic Control]) avoids the restrictive priors of the original formulation [@kim2020bayesian].

Third, the weights are transparent. Every dollar of the synthetic unit comes from an identifiable donor, and the weights themselves carry economic content: they tell you which units the algorithm found most useful for reconstructing the treated unit's pre-period. Compared with DiD, this maximizes similarity between treated and control on both observables and (implicitly) unobservables, and removes the discretion of picking a single comparison group. Compared with a regression-based counterfactual, the convex-combination structure rules out extrapolation: weights live in $[0,1]$ and sum to one, so the synthetic unit cannot wander outside the donor pool's convex hull. And because post-intervention outcomes of the donors are never used in constructing the weights, SCM is structurally protected from the kind of p-hacking that bedevils more flexible estimators.

The price for these properties is a familiar set of trade-offs. The weights, while transparent, can be difficult to defend on substantive grounds — why this 17% from Idaho and 23% from Maine? The convex-hull structure assumes the treated unit's pre-treatment characteristics actually lie inside the donor pool's hull; when the treated unit is on the boundary or outside, the method strains and bias creeps in. And SCM is data-hungry: a long pre-treatment period and a rich donor pool are not luxuries but requirements. In small samples, those assumptions bite hard.

------------------------------------------------------------------------

## Assumptions

The synthetic-control machinery rests on a small set of assumptions that, taken together, justify treating a weighted donor pool as a credible counterfactual for the treated unit. Most of these are familiar siblings of the assumptions behind DiD and matching, but the convex-hull requirement is genuinely particular to SCM and tends to be the one that bites in practice.
